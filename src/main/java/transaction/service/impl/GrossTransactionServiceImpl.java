...
@Service
public class GrossTransactionServiceImpl implements TransactionService {
    ...
    public boolean addTransaction(Transaction transaction) {
        ...
        if (transaction.getTimestamp() < now - (LENGTH * 1000) || transaction.getTimestamp() > now) {
    ...
    // fixme check if fixed rate is correct
    @Scheduled(fixedRate = 1000L)
    void updateOverallStatistics() {
        long now = Instant.now().toEpochMilli();

        int startPosition = (int) ((now - LENGTH * 1000) % LENGTH);
